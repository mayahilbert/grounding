...
var vids = ['https://player.vimeo.com/video/846645071?h=133aa8f5f9',
...
var v = "Time Loaded: " + hours +":"+ minutes + ":" + seconds;
 ...
 document.getElementById("p2").innerHTML = "You are watching hour " + count + " of <br>Teaching Capitalism to Nature.";
...
player.setCurrentTime(minutes*60+seconds).then(function(seconds) {
...
player.ready().then(() => {
  ...
  Promise.all([player.getVideoWidth(), player.getVideoHeight()]).then(
    (dimensions) => {
      ...
      $iframe.height = Math.round((1 / aspectRatio) * window.innerWidth);
    }
  );
});
